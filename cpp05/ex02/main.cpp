...
int main()
{
	...
	try {
		AForm invalid("Invalid Form", 0, 50);
	}
	catch (const std::exception& e) {
		std::cerr << "Exception: " << e.what() << std::endl;
	}
	...
	try {
		AForm invalid("Invalid Form", 50, 151);
	}
	catch (const std::exception& e) {
	...
	try {
		...
		AForm topSecret("Top Secret", 10, 5);
	...
	try {
		...
		AForm exactForm("Exact Form", 50, 50);
